...
def print_h5_structure(h5_file, prefix=''):
    ...
    if h5_file.attrs:
        ...
        for attr_name, attr_val in h5_file.attrs.items():
            ...
            logger.info(f"  - {attr_name}: {attr_val_str}")
    ...
    for key in h5_file.keys():
        ...
        if isinstance(item, h5py.Dataset):
            logger.info(f"{path} (Dataset): Shape={item.shape}, Dtype={item.dtype}")
            # Print attributes of the dataset
            if item.attrs:
                logger.info(f"  Attributes:")
                for attr_name, attr_val in item.attrs.items():
                    logger.info(f"    - {attr_name}: {attr_val}")
        elif isinstance(item, h5py.Group):
            logger.info(f"{path} (Group)")
...
def visualize_sample(group, id_to_phone_map):
    ...
    try:
        ...
        if mel.size == 0:
            logger.error(f"  'mel_spectrograms' dataset not found or empty in group '{filename}'.")
            return
        if f0.size == 0:
            logger.warning(f"  'f0' dataset not found or empty in group '{filename}'.")
        if phone_ids.size == 0:
            logger.error(f"  'phone_sequence' dataset not found or empty in group '{filename}'.")
            return
        if durations.size == 0:
            logger.error(f"  'initial_duration_sequence' dataset not found or empty in group '{filename}'.")
            return
        if len(phone_ids) != len(durations):
             logger.error(f"  Mismatch between length of 'phone_sequence' ({len(phone_ids)}) and 'initial_duration_sequence' ({len(durations)}).")
             return

        logger.info(f"  Mel shape: {mel.shape}")
        logger.info(f"  F0 shape: {f0.shape}")
        logger.info(f"  Phone sequence length: {len(phone_ids)}")
        logger.info(f"  Initial duration sequence length: {len(durations)}")
        ...
        if len(f0) != mel_frames:
             logger.warning(f"  F0 length ({len(f0)}) differs from Mel length ({mel_frames}). Truncating/Padding F0 for plot.")
             if len(f0) > mel_frames:
                 f0 = f0[:mel_frames]
             else:
        ...
        logger.error(f"An error occurred during visualization for {filename}: {e}")
...
def main():
    ...
    if not os.path.exists(args.h5_path):
        logger.error(f"HDF5 file not found at {args.h5_path}")
        return

    try:
        with h5py.File(args.h5_path, 'r') as h5_file:
            logger.info(f"Opened HDF5 file: {args.h5_path}")

            if args.info:
                logger.info("\nHDF5 File Structure:")
                logger.info("--------------------")
                print_h5_structure(h5_file) # Keep using print here as it's for direct structure output
                logger.info("--------------------")
            ...
            try:
                if 'id_to_phone_map' in h5_file.attrs:
                    ...
                    logger.error("'id_to_phone_map' attribute not found in HDF5 file root.")
                    return
            except json.JSONDecodeError:
                logger.error("Failed to decode 'id_to_phone_map' attribute from JSON.")
                return
            except Exception as e:
                 logger.error(f"Error loading id_to_phone_map: {e}")
                 return
            ...
            if args.filename:
                if args.filename in h5_file:
                    ...
                    if not isinstance(target_group, h5py.Group):
                         logger.error(f"'{args.filename}' exists but is not a Group.")
                         return
                else:
                    logger.error(f"Filename (group) '{args.filename}' not found in the HDF5 file.")
                    logger.info("\nAvailable files:")
                    for name in h5_file:
            ...
            if target_group:
                ...
                logger.warning("No target file selected for visualization.")


    except FileNotFoundError:
        logger.error(f"File not found at {args.h5_path}")
    except Exception as e:
...
if __name__ == "__main__":
    setup_logging(level=logging.INFO) # Setup logging
    main()